...
import '../../data/model/account_info_response.dart';


class SateDialogContent extends StatefulWidget implements AutoRouteWrapper {
  String? countryId;
   SateDialogContent({super.key,required this.countryId});

  @override
  State<SateDialogContent> createState() => _SateDialogContentState();
...
class _SateDialogContentState extends State<SateDialogContent> {
  double dialogHeightPercent = 0.7; //0, 1
  @override
  void initState() {
    _fetchStateList(context);
    super.initState();
  }
                          ...
                          child: Column(
                            children: [
                              ...
                              BlocBuilder<AccountInfoBloc, AccountInfoState>(builder: (context, state) {
                                ...
                                } else if (state.infoStatus.isLoadAllStateList) {
                                  return Expanded(
                                      child: ListView(
                                          shrinkWrap: false,
                                          children: (state.allFetchedSateList ?? [] )
                                              .map((e) => Material(
                                            color: Colors.transparent,
                                            child: InkWell(
                                              child: Padding(
  ...
  _fetchStateList(BuildContext context) {
    context.read<AccountInfoBloc>().add( AccountInfoEvent.fetchStateListEvent(countryId:  widget.countryId!));
  }
