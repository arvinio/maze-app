...
@override
  Widget wrappedRoute(BuildContext context) {
    return BlocProvider(create: (_) => inject<CommunityBloc>()..add(const CommunityEvent.getMyCommunitiesEvent()), child: this);
  }
}
class _PostToDialogContentState extends State<PostToDialogContent> {
                              ...
                              SizedBox(
                                  ...
                                  child: BlocBuilder<CommunityBloc,CommunityState>(
                                      builder: (context, state) {
                                        if (state.communityStatus.isMyCommunityLoading) {
                                          return const PageLoading();
                                        } else
                                        if (state.communityStatus.isMyCommunitySuccess) {
                                          communities.clear();
                                          communities.addAll(state.myCommunities!.details!);
                                              ...
                                              itemBuilder: (
                                                ...
                                                return PostToItem(
                                                  title: communities[index]
                                                      .name!,
                                                  subtitle: '${communities[index]
                                                      .memberCount} Members',
                                                  imageUrl: communities[index].avatar,
                                                  onTap: () {
                                                    _onSelect(context,
                                                        communities[index]);
                                                  },
                                        ...
                                        if (state.communityStatus.isMyCommunityFailure) {
  ...
  _fetchUserCommunities(BuildContext context) {
    context.read<CommunityBloc>().add(const CommunityEvent.getMyCommunitiesEvent());
  }
  ...
  void _filterCommunity() {
    if(_searchController.text.isNotEmpty) {
      ...
      filteredCommunities.addAll(
          communities.where((item) => item.name!.toLowerCase().contains(query))
              .toList());
      communities.clear();
    ...
    return Expanded(
        child: Center(
          child: Column(
            ...
            children: [
              CustomText(appStrings.emptyList,
                textAlign: TextAlign.center,style: context.titleHeadline.copyWith(color: context.scheme().disabledText),),
