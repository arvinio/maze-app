...
import 'edit_community_banner_dialog_content.dart';

class EditCommunityDialogContent extends StatefulWidget
    implements AutoRouteWrapper {
  const EditCommunityDialogContent({super.key, required this.communityDetails, required this.onEditedCallback,});
  final CommunityDetails communityDetails;
  final Function onEditedCallback;

  @override
  State<EditCommunityDialogContent> createState() =>
...
class _CreateCommunityDialogContentState
    extends State<EditCommunityDialogContent> {
  ...
  void initState() {
    ...
    _nameController.text = widget.communityDetails.name ?? '';
    _descController.text = widget.communityDetails.description ?? '';
  }
  ...
  Padding _buildContinueButton() {
    return Padding(
      ...
      child: CustomButton.submit(
        ...
        onPressed: () {
          _showEditBanner(context);
        },
      ),
    );
  }

  void _showEditBanner(BuildContext context) {
    showModalBottomSheet(
        isScrollControlled: true,
        context: context,
        backgroundColor: Colors.transparent,
        builder: (builder) {
          final communityDetails = widget.communityDetails.copyWith(
            name: _nameController.text.trim(),
            description:  _descController.text.trim(),
          );
          return EditCommunityBannerDialogContent(communityDetails: communityDetails , onEditedCallback: widget.onEditedCallback,).wrappedRoute(context);
        });
