...
class CommunityPostBloc extends Bloc<CommunityPostEvent, CommunityPostState  > {
  ...
  _onPostList(_postList event ,  Emitter<CommunityPostState> emit)async{
    ...
    final apiResponse = await repository.communityPostList(
        request: event.request
    );
    apiResponse.when(completed: (data, int? statusCode) {
      CommunityPostResponse  response = data;
      if(response.success == true){
        emit(state.copyWith(state: CommunityPostStatus.success, communityPostList: response.communityPosts));
      }else{
  ...
  _onLikePost(_likePost event , Emitter<CommunityPostState> emit)async{
    final apiResponse = await repository.likePost(
        postId: event.postId
    );
    apiResponse.when(completed: (data, int? statusCode) {
      CommunityPostResponse  response = data;
      if(response.success == true){
        emit(state.copyWith(state: CommunityPostStatus.likeSuccessful, likeActionPostId:event.postId , communityPostList: state.communityPostList));
      }
    }, error: (apiError) {
    });
  }
  ...
  _onUnLikePost(_unLikePost event , Emitter<CommunityPostState> emit)async{
    final apiResponse = await repository.unLikePost(
        postId: event.postId
    );
    apiResponse.when(completed: (data, int? statusCode) {
      CommunityPostResponse  response = data;
      if(response.success == true){
        emit(state.copyWith(state: CommunityPostStatus.unLikeSuccessful, likeActionPostId: event.postId , communityPostList: state.communityPostList));
      }
    }, error: (apiError) {
    });
  }
