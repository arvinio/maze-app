...
class CommunityPostWidget extends StatefulWidget {
  const CommunityPostWidget({super.key , required this.post});
      ...
      child: Column(children: [
        Row(
          ...
          children: [
            ...
            const SizedBox(width: Dimen.screenSidePadding,),
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('${widget.post.user?.firstName ?? ''}${widget.post.user?.lastName ?? ''}' , style: context.titleHeadline,),
                Row(
                  children: [
                    Text(widget.post.createdDate?.timeAgo ?? '' , style: context.subheadlineSubheadline,),
                    SizedBox(
                      width: 8.w,
                    ),
                    appAssets.dot.svg(),
                    ...
                    Text(widget.post.title ?? '' , style: context.subheadlineSubheadline,),
                  ],
                )
              ],
            ),
        ...
        Column(
          ...
          children: [
           ...
           if(widget.post.photos?.isNotEmpty == true)
             PhotoCarouselWidget(photoUrls: widget.post.photos!, height: 220.h,),
            const SizedBox(
              height: Dimen.screenSidePadding,
            ),
            Container(
                ...
                children: [
                  Expanded(child: GestureDetector(
                    onTap: (){
                      if(widget.post.isLiked == true){
                        context.read<CommunityPostBloc>().add(CommunityPostEvent.unLikePost(postId: widget.post.id!));
                      }else{
                        context.read<CommunityPostBloc>().add(CommunityPostEvent.likePost(postId: widget.post.id!));
                      }
                    ...
                    child: Row(
                      ...
                      children: [
                        appAssets.heartIcon(isLiked: widget.post.isLiked == true).svg(height: 16.h , width: 16.h , ),
                        SizedBox(width: 6.w,),
                        Text(widget.post.likesCount.toString()  , style: context.subheadlineSubheadline,)
                      ],
                    ),
                  )),
                  VerticalDivider(
                  ...
                  Expanded(child: Row(
                    ...
                    children: [
                      ...
                      SizedBox(width: 6.w,),
                      Text(widget.post.commentsCount.toString() , style: context.subheadlineSubheadline,)
                    ],
                  )),
                ],
              ),
