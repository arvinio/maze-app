...
class CommunityBloc extends Bloc<CommunityEvent, CommunityState> {
  ...
  _onGetCommunityDetailsEvent(_GetCommunityDetailsEvent event, Emitter<CommunityState> emit) async {
    ...
    apiResponse.when(completed: (data,int? statusCode){
      ...
      emit(state.copyWith(communityStatus: CommunityStatus.success,details: response));

    }, error: (apiError){
      emit(state.copyWith(communityStatus: CommunityStatus.failure,errorMessage: apiError.message));
    });
  }

  _onGetMyCommunitiesEvent(_GetMyCommunitiesEvent event, Emitter<CommunityState> emit) async {
    ...
    apiResponse.when(completed: (data,int? statusCode){
      ...
      emit(state.copyWith(communityStatus: CommunityStatus.myCommunitySuccess,myCommunities: response));

    }, error: (apiError){
      emit(state.copyWith(communityStatus: CommunityStatus.myCommunityFailure,errorMessage: apiError.message));
    });
  }


  _onGetOtherCommunitiesEvent(_GetOtherCommunitiesEvent event, Emitter<CommunityState> emit) async {
    emit(state.copyWith(communityStatus: CommunityStatus.otherCommunitiesLoading));

    final apiResponse=await repository.otherCommunities();
    apiResponse.when(completed: (data,int? statusCode){

      OtherCommunitiesResponse response=data;
      emit(state.copyWith(communityStatus: CommunityStatus.otherCommunitiesSuccess,otherCommunities: response));

    }, error: (apiError){
      emit(state.copyWith(communityStatus: CommunityStatus.otherCommunitiesFailure,errorMessage: apiError.message));
    });
  }
}
