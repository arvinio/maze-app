...
class ChangePasswordPage extends StatefulWidget implements AutoRouteWrapper {
  ...
  const ChangePasswordPage(
      {super.key, required this.email, required this.currentPass});
        ...
        child: BlocConsumer<ProfileBloc, ProfileState>(
          listener: (context, state) {
            if (state.profileStatus.isSuccess) {
              context.router.popUntilRouteWithName( const AccountSettingsPageRoute().routeName);
          ...
          builder: (context, state) {
            return Column(
              ...
              children: [
                ListTile(
                  title: CustomText( appStrings.newPasswordTitle,
                      style: context.titleTitle1),
                  subtitle: Padding(
                    padding: const EdgeInsets.only(left: 10.0, top: 5),
                    child: Text.rich(CustomTextSpan(
                        textData: appStrings.typeNewPassword,
                  ...
                  child: CustomButton.submit(
                    ...
                    onPressed: () {
                      context.read<ProfileBloc>().add(
                          ProfileEvent.changePasswordEvent(
                            currentPassword: widget.currentPass,
